...
def setup_logging_and_results_dir() -> Tuple[str, Console]:
    ...
    console.print(Panel(
        f"INICIANDO NOVA EXECUÇÃO\nResultados salvos em: [cyan]{run_results_path}[/cyan]",
        title="[bold green]Otimizador de CBR[/bold green]", expand=False
    ))
...
def run_optimization(console: Console, eval_data: Dict[str, Any]) -> Tuple[Dict[str, Any], float]:
    ...
    best_gene, best_score, history = tuner.run()

    console.print("\n[bold]Resumo do Progresso das Gerações:[/bold]")
    for item in history:
        console.print(
            f"  Geração {item['generation']}/{item['total_generations']} | "
            f"Melhor R²: [bold yellow]{item['best_r2_in_gen']:.4f}[/bold yellow]"
        )
...
def main() -> None:
    ...
    best_config, _ = run_optimization(console, eval_data)
